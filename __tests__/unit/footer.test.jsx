...
import { Footer } from '../../layouts/components/layout/Footer'
...
describe('A1_Component > Footer', () => {
  ...
  test('[A1_1_2]更新履歴の文字があるか', () => {
    render(<Footer />)
    expect(screen.getByText('更新履歴'))
  })

  test('[A1_1_3]aboutの文字があって、リンクが正しいか', () => {
    render(<Footer />)
  ...
  test('[A1_1_4] Manga Studyとはの文字があって、リンクが正しいか', () => {
    render(<Footer />)
    const mangaStudyLink = screen.getByText('Manga Studyとは')
    expect(mangaStudyLink).toBeVisible()
    expect(mangaStudyLink.href).toBe('http://localhost/top')
  })

  test('[A1_1_5] Manga Study ©︎があって、リンクが正しいか', () => {
    render(<Footer />)
